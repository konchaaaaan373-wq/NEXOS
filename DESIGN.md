...
### 禁止タイポグラフィパターン
- 同一見出し内でのfont-light + font-bold混在（例: 通常テキスト + `<span class="font-bold">強調</span>`）
- 見出しは1つのウェイトで統一する。部分的な太字切り替えはAI生成の典型パターン
- 見出しのウェイトは **font-bold 単体** または **font-light 単体** のどちらかに統一する
